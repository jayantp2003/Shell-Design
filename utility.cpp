...
const char *history_file = "/tmp/cmd_history";
...
void read_history() {
    ...
    // Try to open the history file in read mode
    fphist = fopen(history_file, "r");

    // If the file is not found, create it and open it in write mode
    if (fphist == NULL) {
        // Create the file
        fphist = fopen(history_file, "w");
        if (fphist == NULL) {
            // If the file cannot be created, print an error message and return
            perror("Error creating history file");
            return;
        }
        // Close the file (it will be reopened in read mode later)
        fclose(fphist);
    }
    ...
    // If the file is still not found, print an error message and return
    if (fphist == NULL) {
